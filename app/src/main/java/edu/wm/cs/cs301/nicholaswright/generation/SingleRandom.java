...
public final class SingleRandom {
	...
	public static void setSeed(final int seed) {
		if (null != instance) {
			Log.v("SingleRandom", "Warning: SingleRandom already instantiated, resetting seed with value " + seed);
			instance.generator.setSeed(seed) ;
		}
		theSeed = seed;
		validSeed = true;
		Log.v("SingleRandom", "Setting seed to " + seed);
	}
	
	/**
	 * If a seed has been set, this method gives it
	 * @return the seed associated with this class
	 */
	public int getSeed() {
		if(validSeed) {
			return theSeed;
			...
			Log.v("SingleRandom", "Seed has not been set");
			return 0;
		}
	}
}
